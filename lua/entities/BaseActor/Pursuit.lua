...
ACTOR_PURSUIT_DATA = {}
local ACTOR_PURSUIT_DATA = ACTOR_PURSUIT_DATA
...
ENT.tNoFactionPursuitData = {}

// TODO: We might want to get their GetRunSpeed() or flRunSpeed if they have those
// to see if they're sprinting away

function ENT:UpdatePursuitSenses( pEnemy, pKey /* a.k.a pTrueEnemy */ )
	if !IsValid( pKey ) || !pKey:IsPlayer() /* dirty HACK */ then return end

	local PURSUIT_DATA = ACTOR_PURSUIT_DATA[ self:Classify() ] || self.tNoFactionPursuitData
	...
	if !tData then
		tData = {
			flRunningAwayFactor = 0,
			vVelocity = Vector()
		}
		PURSUIT_DATA[ pKey ] = tData
	end

	tData.flTime = SysTime()

	local vVelocity = GetVelocity( pEnemy )

	local f = vVelocity:GetNormalized():Dot( ( pEnemy:GetPos() + pEnemy:OBBCenter() - self:GetPos() ):GetNormalized() )
	...
	tData.vVelocity = LerpVector( FRILerpRate( .05, FrameTime() ), tData.vVelocity, vVelocity * f )

	return f >= 2 / 3, tData.vVelocity
end
