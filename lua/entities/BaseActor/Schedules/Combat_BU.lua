...
Actor_RegisterSchedule( "Combat", function( self, sched, MyTable )
	...
	if MyTable.flCombatState > 0 && LevelOfDetail( sched, "flNextSearch" ) then
		...
		if !tQueue then
			...
			flBestCandidate = ( flNewCost - self:BoundingRadius() ) * math.Remap( self.flCombatState, 0, 1, 1, .33 )
		...
		while !table_IsEmpty( tQueue ) do
			...
			pPath:MoveCursorToClosestPosition( LerpVector( .5, vStart, vEnd ) )
			local iCursor = pPath:GetCursorPosition()
			...
			local tPitchAngles = enemy:GetPos().z > self:GetPos().z && ACTOR_PITCH_ANGLES_SHORT_UP || ACTOR_PITCH_ANGLES_SHORT_DOWN
			local flDistSqr = math.max( vStart:Distance( enemy:GetPos() ), vEnd:Distance( enemy:GetPos() ) )
			...
			if !( !util_TraceLine( {
				...
				filter = tFilter,
				mask = MASK_SHOT_HULL
			} ).Hit && fDo( vLeftCheckDucked, ACTOR_PITCH_ANGLES_SHORT_LEFT ) ||
			!util_TraceLine( {
				start = vEnd + Vector( 0, 0, MyTable.vHullDuckMaxs[ 1 ] ) + dDirection:Angle():Right() * ( bRight && -flSize || flSize ),
				endpos = vRightCheckDucked,
				filter = tFilter,
				mask = MASK_SHOT_HULL
			} ).Hit && fDo( vRightCheckDucked, ACTOR_PITCH_ANGLES_SHORT_RIGHT ) ) then continue end
			//	if !( !util_TraceLine( {
			//		start = vStart + Vector( 0, 0, MyTable.vHullDuckMaxs[ 1 ] ) + dDirection:Angle():Right() * ( bRight && -flSize || flSize ),
			//		endpos = vLeftCheckDucked,
			//		filter = tFilter,
