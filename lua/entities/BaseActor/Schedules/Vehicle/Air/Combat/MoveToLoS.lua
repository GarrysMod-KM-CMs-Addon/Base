RegisterSchedule( "VehicleAirMoveToLoS", { Execute = function( self, pSchedule, MyTable )
	...
	if !pSchedule.vStart then pSchedule.vStart = self:GetPos() end

	local vEnemy = pEnemy:GetPos() + pEnemy:OBBCenter()
	local vCenter = pVehicle:GetPos() + pVehicle:OBBCenter()
	local vPoint = pSchedule.vPoint

	if !MyTable.bHoldFire && pVehicle:MachineGunWeaponCanHit( vEnemy, pTrueEnemy ) then
		...
		if pVehicle:MachineGunWeaponHits( vEnemy, pTrueEnemy ) then
