	...
	if MyTable.flCombatState > 0 && !sched.bBeganAdvanceSearching then
		...
		ACTOR_QUEUE( function()
			...
			if bCanLandAShot then
				...
				local _, vPos = util_DistanceToLine( MyTable.tCover[ 1 ], MyTable.tCover[ 2 ], vEnemy )
				pPath:MoveCursorToClosestPosition( vPos )
				local flBestCandidate = math.huge
				...
				while !table_IsEmpty( tQueue ) do
					...
					if !util_TraceLine( {
						...
						b = util_TraceLine( {
							...
							endpos = util_TraceLine( {
								start = vTarget,
								endpos = vTarget + ( vTarget - vLeftCheckDucked ):Angle():Right() * self:BoundingRadius() * 4,
								mask = MASK_SHOT_HULL,
								filter = tFilter
							} ).HitPos,
							mask = MASK_SHOT_HULL,
					...
					if b && !util_TraceLine( {
						...
						b = util_TraceLine( {
							...
							endpos = util_TraceLine( {
								start = vTarget,
								endpos = vTarget + ( vTarget - vRightCheckDucked ):Angle():Right() * self:BoundingRadius() * 4,
								mask = MASK_SHOT_HULL,
								filter = tFilter
							} ).HitPos,
							mask = MASK_SHOT_HULL,
