...
function EFFECT:Init( pData )
	...
	local bCustomPosition = fGetMuzzleFlashPosition && fGetMuzzleFlashPosition( pEntity, "MuzzleFlashGeneric", iGetMaterialIndex )
	if bCustomPosition then
		local iGetMaterialIndex = pData:GetMaterialIndex()
		fGetPos = function()
			if !IsValid( pEntity ) then return vector_origin end
			return fGetMuzzleFlashPosition( pEntity, "MuzzleFlashGeneric", iGetMaterialIndex ) || vector_origin
		end
	end

	local fGetMuzzleFlashAngles = pEntity.GetMuzzleFlashAngles
	local bCustomAngles = fGetMuzzleFlashAngles && fGetMuzzleFlashAngles( pEntity, "MuzzleFlashGeneric", iGetMaterialIndex )
	if bCustomAngles then
		local iGetMaterialIndex = pData:GetMaterialIndex()
		fGetAngles = function()
			if !IsValid( pEntity ) then return angle_zero end
			return fGetMuzzleFlashAngles( pEntity, "MuzzleFlashGeneric", iGetMaterialIndex ) || angle_zero
		end
	end
	...
	for i = 1, 8 do
		local pPart = pEmitter:Add( bFirstPerson && "MuzzleFlashHeatHazeView" || "MuzzleFlashHeatHazeWorld", fGetPos() )
